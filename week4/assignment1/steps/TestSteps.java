package week4.assignment1.steps;
...
//	(Short-desc - "This is Third" and category -"inquiry") and verify the short-desc is not empty
	...
	@Then("validate the response for below {string} {string} & {string} & {string}")
	public void validateResposeStringForMulti(String order, String key , String value, String value1) {
		if (order =="1") {response.then().body(key, Matchers.equalTo(value));}	
		if (order =="2") {response.then().body(key, Matchers.anyOf(Matchers.not(value),Matchers.not(value1)));}	
		if (order =="3") {response.then().body(key, Matchers.not(Matchers.emptyOrNullString()));}
